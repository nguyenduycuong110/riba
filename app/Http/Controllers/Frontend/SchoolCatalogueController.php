...
class SchoolCatalogueController extends FrontendController
{
    ...
    {
        $catName = $schoolCatalogue->languages->first()->pivot->name ?? '';
        $catCanonical = write_url($schoolCatalogue->languages->first()->pivot->canonical ?? '');
        $catDescription = strip_tags($schoolCatalogue->languages->first()->pivot->description ?? '');

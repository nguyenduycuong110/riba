...
class AdmissionCatalogueController extends FrontendController
{
    ...
    private function schema($admissionCatalogue, $admissions, $breadcrumb)
    {
        ...
        $cat_name = $admissionCatalogue->languages->first()->pivot->name;

        $cat_canonical = write_url($admissionCatalogue->languages->first()->pivot->canonical);

        $cat_description = strip_tags($admissionCatalogue->languages->first()->pivot->description);
            ...
            $itemListElements .= "
                {
                    ...
                    \"headline\": \" " . $name . " \",
                    \"url\": \" " . $canonical . " \",
                    \"datePublished\": \" " . convertDateTime($admission->created_at, 'd-m-Y') . " \",
                    \"author\": {
                        \"@type\": \" Person  \",
                        \"name\": \" An Hưng \",
                    }

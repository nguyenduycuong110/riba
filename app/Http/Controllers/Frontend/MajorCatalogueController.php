...
class MajorCatalogueController extends FrontendController
{
    ...
    {
        $catName = $majorCatalogue->languages->first()->pivot->name ?? '';
        $catCanonical = write_url($majorCatalogue->languages->first()->pivot->canonical ?? '');
        $catDescription = strip_tags($majorCatalogue->languages->first()->pivot->description ?? '');
        ...
        foreach ($majors as $major) {
            ...
            $datePublished = $major->created_at ? convertDateTime($major->created_at, 'd-m-Y') : '';
